...
class Board:
    ...
    def __init__(self, N, pos_black, pos_white, pos_arrows):
        ...
        startingArrows = []
        for i in pos_arrows:
            startingArrows.append(Pos2D.from_string(i))
        self.queens = [list(map(Pos2D.from_string, positions)) for positions in (pos_white, pos_black)]
        self.startingSetup = N, copy(self.queens[1]), copy(self.queens[0]), startingArrows
    ...
    def is_simple(self, component):
        ...
        * C est un triangle de type 2 ssi |E| = 3 et n = m^2 et M = 2m-1 pour m = min{H, W} et M = max{H, W}.
        ...
        if w == 1 and n == h or h == 1 and n == w:  # H/V line
            ...
            is_simple = True
        elif len(convex_hull) == 3:  # triangle
    ...
    def act(self, action, log_action=True):
        ...
        if not self.is_valid_action(action):
            raise InvalidActionError(f'{action} n\'est pas une action valide pour le joueur {action.player_id}')
    ...
    def _possible_moves_from(self, origin, ignore=None):
        """
        Énumère les déplacements possibles depuis une certaine position sur la plateau
        ...
        for direction in DIRECTIONS:
            ...
            while self.is_valid_pos(new_pos) and (self.at(new_pos) == EMPTY or (ignore is not None and new_pos == ignore)):
                yield new_pos.copy()
                new_pos += direction
